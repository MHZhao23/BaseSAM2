...
class VideoEvaluator:
    ...
    def consolidate(
        ...
        for obj_id, iou, boundary_f, dice in eval_results:
            if len(iou) == 0:
                iou_output[obj_id] = np.nan
                boundary_f_output[obj_id] = np.nan
                dice_output[obj_id] = np.nan
            else:
                assert len(iou) == 1
                key = list(iou.keys())[0]
...
def benchmark(
    ...
    *,
    ...
    for i, mask_root in enumerate(mask_roots):
        ...
        global_j = np.nanmean(np.array(all_iou))
        global_f = np.nanmean(np.array(all_boundary_f))
        global_jf = (global_j + global_f) / 2
        global_dice = np.nanmean(np.array(all_dice))
